...
**当前问题**: `wire.go:14` 使用内存 map，重启丢失

**任务清单**:

- [ ] 创建数据库迁移脚本 `db/migrations/000X_device_params_up.sql`
- [ ] 实现 `Repository.StoreParamWrite()` 数据库版本
- [ ] 实现 `Repository.GetParamWritePending()` 数据库版本
- [ ] 移除 `wire.go` 中的内存 `paramStore`
- [ ] 添加单元测试 `param_persistence_test.go`
...
**协议要求**: 支持完整的刷卡充电流程（7步流程，3种充电模式）

**重要提醒**: ⚠️ **命令 0x0F (设备回应订单) 容易遗漏，但是必需的！**
...
- [ ] 创建 `cards` 表 (卡号、余额、状态)
- [ ] 扩展 `orders` 表 (card_no, charging_mode, spent_cents)
- [ ] 创建 `card_transactions` 表 (卡片交易记录)

#### 2.2 协议编解码

- [ ] 实现 `ParseCardReport()` - 解析设备上报卡号（cmd=0x0B上行，461-478行）
- [ ] 实现 `EncodeCardChargingCommand()` - 编码充电指令（cmd=0x0B下行）
  - [ ] 支持按时/按量模式（480-496行）
  - [ ] 支持按功率模式（498-521行，含档位配置）
- [ ] ⭐ **实现 `ParseOrderConfirmation()` - 解析设备回应订单（cmd=0x0F上行，522-534行）**
- [ ] 实现 `ParseCardChargingEnd()` - 解析充电结束（cmd=0x0C上行）
  - [ ] 按时/按量格式（536-563行）
  - [ ] 按功率格式（564-585行，含每档时间）
- [ ] 实现 `EncodeCardChargingEndACK()` - 编码结束响应（cmd=0x0C下行，587-597行）

#### 2.3 业务处理

- [ ] 实现 `HandleCardCharging()` - 完整刷卡充电处理器
  - [ ] 卡片验证逻辑
  - [ ] 余额检查
  - [ ] 创建充电订单（支持3种模式）
  - [ ] 更新端口状态
- [ ] ⭐ **实现 `HandleOrderConfirmation()` - 处理设备订单确认（cmd=0x0F）**
  - [ ] 更新订单状态为"设备已确认"
  - [ ] 失败时触发订单回滚
- [ ] 实现 `HandleCardChargingEnd()` - 充电结束处理
  - [ ] 订单结算（区分按时/按量/按功率）
  - [ ] 余额扣减
  - [ ] 生成交易记录
  - [ ] 按功率模式：记录每档充电时间
...
- [ ] 实现 `HandleCardBalanceQuery()` - 余额查询处理
- [ ] 实现 `EncodeCardBalanceResponse()` - 编码余额响应

#### 2.5 测试

- [ ] 创建 `card_charging_test.go`
- [ ] 测试完整流程：上报 → 验证 → 充电 → 结束
- [ ] 测试余额不足场景
- [ ] 测试离线卡/在线卡区分
...
- 2.2.3 刷卡充电 (454-597行)
- 重点关注：cmd=0x0F 设备回应订单 (522-534行) ⭐
- 2.2.4 刷卡查询余额 (599-632行)
...
- [ ] 实现 `EncodeNetworkRefreshCommand()` - 刷新列表（cmd=0x08）
- [ ] 实现 `EncodeNetworkAddNodeCommand()` - 添加插座（cmd=0x09）
- [ ] 实现 `EncodeNetworkDeleteNodeCommand()` - 删除插座（cmd=0x0A）
- [ ] 实现 `ParseNetworkResponse()` - 解析设备响应

#### 3.3 业务处理

- [ ] 实现 `HandleNetworkRefresh()` - 刷新列表处理器
- [ ] 实现 `HandleNetworkAddNode()` - 添加节点处理器
- [ ] 实现 `HandleNetworkDeleteNode()` - 删除节点处理器
- [ ] 在 `router_handlers.go` 注册命令处理器

#### 3.4 API接口

- [ ] 添加 HTTP API: `POST /api/gateway/{id}/sockets/refresh`
- [ ] 添加 HTTP API: `POST /api/gateway/{id}/sockets`
- [ ] 添加 HTTP API: `DELETE /api/gateway/{id}/sockets/{socket_no}`

#### 3.5 测试

- [ ] 创建 `network_management_test.go`
- [ ] 测试批量刷新列表
- [ ] 测试添加单个插座
- [ ] 测试删除单个插座
...
- [ ] 实现 `EncodeOTACommand()` - 编码OTA升级指令（cmd=0x07）
  ...
  type OTACommand struct {
      ...
      FTPPort    uint16
      FileName   string // 13字节ASCII hex
  }
  ```

- [ ] 实现 `ParseOTAResponse()` - 解析设备响应
- [ ] 实现 `ParseOTAProgress()` - 解析升级进度上报

#### 4.3 业务处理

- [ ] 实现 `HandleOTA()` - OTA升级处理器
- [ ] 实现 `HandleOTAProgress()` - 升级进度处理
- [ ] 在 `router_handlers.go` 更新 0x0007 注册

#### 4.4 API接口

- [ ] 添加 HTTP API: `POST /api/devices/{id}/ota`
- [ ] 添加 HTTP API: `GET /api/devices/{id}/ota/status`
...
| Phase | 工作量 | 说明 |
|-------|--------|------|
| Phase 1 | 12-13天 | 高优先级，必须完成（刷卡+1天） |
| Phase 2 | 7天 | 增强功能，建议完成 |
| Phase 3 | 4.5天 | 优化项（新增状态位+约束验证） |
| Phase 4 | 5-7天 | 测试与文档 |
| **总计** | **28.5-31.5天** | 约 **6周** |
...
- [ ] 平台能验证卡片并下发充电指令（cmd=0x0B下行，支持3种模式）
- [ ] ⭐ 设备能回应订单确认（cmd=0x0F）
- [ ] 充电过程中能查询余额（cmd=0x1A）
- [ ] 充电结束能正确扣减余额并结算（cmd=0x0C，区分两种格式）
