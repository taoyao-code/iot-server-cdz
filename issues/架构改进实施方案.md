...
| # | 任务 | 工作量 | 责任人 | 状态 |
|---|-----|--------|--------|------|
| 1 | 修复启动顺序问题 | 0.5天 | 待分配 | ⏳ 待开始 |
| 2 | 实现参数持久化存储 | 1天 | 待分配 | ⏳ 待开始 |
| 3 | 添加API认证 | 2-3天 | 待分配 | ⏳ 待开始 |
| 4 | 实现会话Redis化 | 3-5天 | 待分配 | ⏳ 待开始 |

---

### 2.2 任务1: 修复启动顺序 ⚡

**问题**: TCP启动早于数据库连接，导致早期消息丢失
...
func Run(cfg *cfgpkg.Config, log *zap.Logger) error {
    ...
    if err != nil {
        return fmt.Errorf("database initialization failed: %w", err)  // ✅ 失败直接返回
    }
    defer dbpool.Close()
...
COMMENT ON TABLE device_params IS 'BKV设备参数写入记录';
COMMENT ON COLUMN device_params.status IS '0=待确认, 1=已确认, 2=失败';
```
...
# 查询数据库
psql -d iot_server -c "SELECT * FROM device_params WHERE device_id = 1;"
# 期望：status=1, confirmed_at不为NULL
```
...
// RegisterReadOnlyRoutes 注册只读API路由
...
api:
  auth:
    enabled: true  # 开发环境可设为false
    api_keys:
      - "sk_live_abcd1234efgh5678"  # 生产环境
      - "sk_test_test1234test5678"  # 测试环境
```
...
type SessionData struct {
    ...
    ServerAddr string    `json:"server_addr"`  // 连接所在服务器地址
    Protocol   string    `json:"protocol"`
    LastSeen   time.Time `json:"last_seen"`
    CreatedAt  time.Time `json:"created_at"`
}

// BindSession 绑定会话到当前服务器
...
// HybridManager 混合会话管理器（内存+Redis）
type HybridManager struct {
    local      *Manager       // 本地内存存储（连接对象）
    remote     *RedisStore    // Redis存储（会话信息）
    serverAddr string         // 当前服务器地址
}

// NewHybridManager 创建混合管理器
...
| 风险 | 概率 | 影响 | 缓解措施 |
|-----|------|------|---------|
| Redis故障导致服务中断 | 中 | 高 | 实现降级机制 |
| 数据迁移失败 | 低 | 高 | 完整备份+回滚脚本 |
| 性能下降 | 中 | 中 | 压测验证+灰度发布 |
| API Key泄露 | 低 | 高 | 定期轮换+IP白名单 |
...
# 回滚方案2: 切回内存会话
# 配置：session.backend = "local"

# 回滚方案3: 禁用API认证（紧急情况）
# 配置：api.auth.enabled = false
```
