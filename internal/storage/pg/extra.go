...
func (r *Repository) GetPendingOrderByPort(ctx context.Context, deviceID int64, portNo int) (*Order, error) {
	const q = `SELECT id, device_id, '' as phy_id, port_no, order_no, business_no, charge_mode, start_time, end_time, kwh_0p01, amount_cent, status
	...
	err := r.Pool.QueryRow(ctx, q, deviceID, portNo).Scan(&ord.ID, &ord.DeviceID, &ord.PhyID, &ord.PortNo, &ord.OrderNo, &biz, &ord.ChargeMode, &ord.StartTime, &ord.EndTime, &kwh, &amt, &ord.Status)
	if err != nil {
...
// GetOrderByBusinessNo 根据业务号查找订单（按device唯一，状态不限）
func (r *Repository) GetOrderByBusinessNo(ctx context.Context, deviceID int64, businessNo uint16) (*Order, error) {
	const q = `SELECT id, device_id, '' as phy_id, port_no, order_no, business_no, charge_mode, start_time, end_time, kwh_0p01, amount_cent, status
		FROM orders WHERE device_id=$1 AND business_no=$2 ORDER BY id DESC LIMIT 1`
	var (
		ord Order
		kwh *int64
		amt *int64
	...
	err := r.Pool.QueryRow(ctx, q, deviceID, int64(businessNo)).Scan(
		&ord.ID,
		&ord.DeviceID,
		&ord.PhyID,
		&ord.PortNo,
...
func (r *Repository) GetInterruptedOrders(ctx context.Context, deviceID int64) ([]Order, error) {
	rows, err := r.Pool.Query(ctx, `SELECT id, device_id, '' as phy_id, port_no, order_no, business_no, charge_mode, start_time, end_time, kwh_0p01, amount_cent, status
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ord.ID, &ord.DeviceID, &ord.PhyID, &ord.PortNo, &ord.OrderNo, &biz, &ord.ChargeMode, &ord.StartTime, &ord.EndTime, &kwh, &amt, &ord.Status); err != nil {
			return nil, err
		}
		ord.Kwh01 = kwh
		ord.AmountCent = amt
