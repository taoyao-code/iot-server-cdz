...
- 代码路径：
  - TCP 网关：`internal/tcpserver/server.go`
  - 协议路由占位：`internal/protocol/ap3000/router.go`
  - 协议解析：`internal/protocol/ap3000/{frame.go,parser.go,router_table.go}`
  - HTTP 健康与指标：`internal/httpserver/server.go`
  - 配置加载：`internal/config/config.go`
  - 日志初始化：`internal/logging/logger.go`
  - 指标注册：`internal/metrics/metrics.go`

- 配置键（对应 `configs/example.yaml`）：
  - `tcp.addr`, `tcp.readTimeout`, `tcp.writeTimeout`, `tcp.maxConnections`, `tcp.connectionBacklog`
  - `metrics.enable`, `metrics.path`
  - `logging.level`, `logging.format`, `logging.file.*`
  - `app.*`, `http.*`, `database.*`

- 运行方式：
  - 本地：`make run`（默认读取 `IOT_CONFIG=./configs/example.yaml`）
  - 健康检查：`/healthz`、就绪：`/readyz`、指标：`/metrics`
  - 数据接线：TCP → 解析（AP3000）→ 路由（按 cmd）已在 `cmd/server/main.go` 里对接
