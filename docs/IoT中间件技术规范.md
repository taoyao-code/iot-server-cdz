...
```mermaid
graph LR
    A[第三方业务系统] -->|HTTP API| B[IoT中间件<br/>本系统]
    B -->|TCP/BKV| C[充电设备]
    A -.负责.- A1[用户认证<br/>支付扣款<br/>账户管理]
    B -.负责.- B1[设备连接<br/>协议转换<br/>状态同步]
    C -.负责.- C1[充电执行<br/>状态上报<br/>实时监控]
```
...
graph TB
    subgraph "第三方业务系统"
        TP[第三方API调用方<br/>已完成扣款]
    end

    subgraph "IoT中间件系统 本系统"
        HTTP[HTTP API层<br/>internal/api]
        TCP[TCP Server<br/>internal/tcpserver]
        Session[会话管理<br/>internal/session<br/>Redis]
        Queue[下行队列<br/>internal/outbound<br/>PostgreSQL]
        DB[(PostgreSQL<br/>设备/订单/事件)]
        Push[事件推送<br/>internal/thirdparty]
    end

    subgraph "充电设备"
        Device[BKV设备<br/>30秒心跳]
    end

    TP -->|1. POST /v1/order/create| HTTP
    HTTP -->|2. 检查设备在线| Session
    Session -->|3. last_seen < 60s?| DB
    HTTP -->|4. 创建订单| DB
    HTTP -->|5. 入队指令| Queue
    Queue -->|6. TCP发送| TCP
    TCP -->|7. 0x1011控制| Device
    Device -->|8. ACK| TCP
    TCP -->|9. 更新状态| DB
    DB -->|10. 触发推送| Push
...
如果在线阈值设置为 30 秒，可能导致设备误判离线。

**修复建议**：

- 在线阈值设为 **60 秒**（心跳周期 × 2）
- 配置文件：`device.online_threshold = 60`
- 避免因网络波动导致误判
...
stateDiagram-v2
    ...
    Connected --> Online: 收到心跳(0x1007)
    Online --> Online: 持续心跳(30s)
    Online --> Offline: 心跳超时(>60s)
    Offline --> Online: 恢复心跳
    Connected --> Disconnected: TCP断开
    Offline --> Disconnected: TCP断开
```
...
stateDiagram-v2
    [*] --> free: 初始状态
    free --> occupied: 插枪(0x100F)
    occupied --> charging: 开始充电(0x1011)
    charging --> occupied: 停止充电
    occupied --> free: 拔枪
    free --> fault: 故障
    occupied --> fault: 故障
    charging --> fault: 故障
...
stateDiagram-v2
    ...
    pending --> confirmed: 设备ACK确认(10秒内)
    pending --> timeout: 设备无响应(10秒)
    pending --> cancelling: 第三方取消

    confirmed --> charging: 开始充电
    confirmed --> cancelling: 第三方取消

    charging --> interrupted: 设备离线
    charging --> stopping: 第三方停止(唯一途径)
    charging --> completed: 正常结束
    charging --> failed: 设备故障

    interrupted --> charging: 60秒内恢复
    interrupted --> failed: 60秒未恢复

    cancelling --> cancelled: 设备ACK确认
    cancelling --> cancelled: 30秒超时自动取消
    cancelling --> completed: 设备已完成(竞态处理)
    cancelling --> failed: 设备故障

    stopping --> stopped: 设备ACK确认
    stopping --> stopped: 30秒超时自动停止
    stopping --> completed: 设备已完成(竞态处理)
    stopping --> failed: 设备故障
...
sequenceDiagram
    ...
    DB-->>API: {online: false, last_seen: 125秒前}

    Note over API: ❌ 检查失败：设备离线
    API-->>TP: 503 Service Unavailable<br/>{<br/>  "code": 50301,<br/>  "message": "device is offline",<br/>  "data": {<br/>    "device_id": "82241218000382",<br/>    "last_seen": "2分钟前",<br/>    "suggestion": "请选择其他设备"<br/>  }<br/>}
```
...
sequenceDiagram
    ...
    Note over Dev: T0: 充电正常进行
    Dev->>TCP: 心跳 0x1007
    TCP->>DB: last_seen_at = T0

    Note over Dev: T0+60s: 设备断电

    Note over TCP,Monitor: T0+90s: 心跳超时检测
...
```mermaid
sequenceDiagram
    participant TP1 as 第三方A
    participant TP2 as 第三方B
    participant API as API
    participant DB as DB
    ...
    Note over API,DB: 使用数据库行锁
    API->>DB: BEGIN TRANSACTION
    API->>DB: SELECT * FROM orders<br/>WHERE device_id=123 AND port_no=1<br/>AND status IN (0,1,2)<br/>FOR UPDATE  -- 行锁
    ...
    Note over TP2: 等待行锁释放

    API->>DB: BEGIN TRANSACTION
    API->>DB: SELECT ... FOR UPDATE
    DB-->>API: 1 row (已被占用)
    API->>DB: ROLLBACK
    API-->>TP2: 409 Conflict<br/>{<br/>  "code": 40901,<br/>  "message": "port is occupied",<br/>  "data": {<br/>    "busy_order": "THD111...",<br/>    "suggestion": "请选择其他端口"<br/>  }<br/>}
```
...
- 配置文件修改：`device.online_threshold = 60`（心跳周期 × 2）
- 避免因网络波动导致误判
...
代码位置：`internal/protocol/bkv/handlers.go::HandleACK()`
...
代码位置：`internal/storage/pg/order_repo.go::CreateOrder()`
...
1. **创建订单前强制查询实时端口状态**：
   - 下发 0x1012 查询命令（同步等待响应，超时 5 秒）
   - 验证设备返回的端口状态与数据库一致
   - 不一致时拒绝创单，返回错误码 `PORT_STATE_MISMATCH`
   - 触发异步状态同步任务（见方案 3）
2. **订单状态变更后验证端口状态**：
   - **启动场景**：`confirmed` → `charging` 后 30 秒内
   - **停止场景**：下发停止指令后 30 秒内
   - 查询端口状态（0x1012），验证与预期一致：
     - 启动：设备端口应为 `charging(1)`
     - 停止：设备端口应为 `free(0)`
   - 不一致时触发告警+人工介入（见方案 4）
3. **定期状态同步任务**（每 5 分钟）：
   - 查询所有在线设备的全部端口状态（0x1012）
   - 对比数据库状态，发现差异时：
     - **端口 free 但订单 charging**：记录告警，人工核实后决定是否关闭订单
     - **端口 charging 但无订单**：下发停止指令（0x1010），清理端口
     - **端口 charging 且订单不匹配**：告警，禁止新订单创建（标记端口异常）
4. **状态不一致告警与恢复**：
   - Prometheus 指标：`port_state_mismatch_total{reason="free_vs_charging|charging_vs_free|charging_vs_other"}`
   - 记录详细日志：设备 ID、端口号、数据库状态、设备状态、关联订单号
   - **自动恢复条件**：连续 3 次同步任务状态一致，解除告警
   - **人工介入条件**：不一致持续超过 15 分钟

代码位置：

- `internal/api/thirdparty_handler.go::CreateOrder()` - 实时查询
- `internal/service/port_sync.go::SyncPortStates()` - 定期同步
...
2. **取消/停止流程改造**：
   ...
   ```
   a) 第三方发起取消请求
   b) 使用CAS更新：WHERE status IN (0,1) SET status=8 (cancelling)
   c) 下发取消指令到设备（0x1013）
   d) 设备ACK确认 → 更新为cancelled(5)
   e) 设备ACK时检查：if status==cancelling then reject start
   ```
...
3. **处理 ACK 时的状态检查**：
   ...
   if order.Status == cancelling {
       // 拒绝启动，保持cancelling状态
       // 等待取消指令ACK
       return ErrOrderCancelling
   }
   ```

4. **超时保护**：
   - cancelling/stopping 超过 30 秒未收到 ACK
   - 强制标记为 cancelled/stopped

代码位置：

- `internal/api/thirdparty_handler.go::CancelOrder()`
- `internal/protocol/bkv/handlers.go::HandleACK()`
...
1. **指令优先级分级**：
   ...
   ```
   - 紧急(priority=1): stop(停止), cancel(取消)
   - 控制(priority=2): start(启动), query(查询)
   - 低优(priority=3): heartbeat_ack(心跳响应)
   ```

2. **多队列架构**：
   - 数据库增加字段：`priority INT DEFAULT 2`
   - 每个设备维护 3 个逻辑队列（高/中/低）
   - 调度策略：高优先级队列优先处理，低优队列限流
3. **死信队列机制**：
   - 指令失败 3 次后移入`outbound_queue_dlq`表
   - 标记失败原因：`device_offline`, `timeout`, `ack_error`
   - 每小时清理超过 24 小时的死信记录
4. **降级策略**：

   - 队列长度>200 时：拒绝 priority=3 的新指令
   - 队列长度>500 时：拒绝 priority=2 的查询类指令
   - 队列长度>1000 时：触发 P0 告警，只接受 priority=1 指令
...
- `internal/outbound/queue.go::Enqueue()` - 优先级入队
- `internal/outbound/scheduler.go::Schedule()` - 多队列调度
- `internal/outbound/dlq.go::MoveToDLQ()` - 死信处理
...
1. **引入 Outbox 模式**：
   ...
   CREATE TABLE events (
       ...
       event_type VARCHAR(32) NOT NULL,  -- charging_start, progress, completed
       event_data JSONB NOT NULL,
       sequence_no INT NOT NULL,         -- 事件序列号
       status INT DEFAULT 0,             -- 0:待推送, 1:已推送, 2:失败
...
2. **订单状态更新与事件写入在同一事务**：
   ...
   ```go
   tx.Begin()
   tx.UpdateOrder(orderNo, status)
   tx.InsertEvent(orderNo, eventType, data, sequenceNo)
   tx.Commit()
   ```
...
3. **后台事件推送任务**（每 10 秒）：
   ...
   ```go
   - SELECT * FROM events WHERE status=0 OR (status=2 AND retry_count<5)
   - 按order_no + sequence_no排序推送
   - 推送成功：status=1, pushed_at=NOW()
   - 推送失败：status=2, retry_count+=1
   - retry_count>=5：status=3(永久失败), 触发告警
   ```
...
5. **提供 HTTP 查询接口兜底**：
   ...
   ```
   GET /v1/order/{order_no}/events
   返回该订单的所有事件列表（按序列号排序）
   第三方可主动查询补偿
   ```

6. **推送失败告警**：
   - Prometheus 指标：`event_push_failed_total`
   - retry_count>=3 时发送 P1 告警
   - 永久失败时发送 P0 告警

代码位置：

- `internal/storage/pg/order_repo.go::UpdateStatus()` - 事务写入
- `internal/thirdparty/event_pusher.go::PushEvents()` - 后台推送
- `internal/api/readonly_handler.go::GetOrderEvents()` - 查询接口
...
graph LR
    subgraph "设备监控"
        A1[设备在线率<br/>target: >95%]
        A2[心跳超时率<br/>alert: >5%]
        A3[设备故障率<br/>alert: >2%]
    end

    subgraph "订单监控"
        B1[订单创建成功率<br/>target: >98%]
        B2[订单超时率<br/>alert: >5%]
        B3[设备ACK平均时延<br/>alert: >5s]
    end

    subgraph "系统监控"
        C1[API响应时间<br/>P99 <200ms]
        C2[TCP连接数<br/>monitoring]
        C3[下行队列长度<br/>alert: >100]
    end
```
