# IoT 充电桩 E2E 测试套件

完整、规范的端到端(E2E)测试套件，用于验证 IoT 充电桩系统的第三方 API 功能。

## 📋 目录

- [特性](#特性)
- [环境要求](#环境要求)
- [快速开始](#快速开始)
- [配置](#配置)
- [运行测试](#运行测试)
- [测试套件](#测试套件)
- [CI/CD 集成](#cicd集成)
- [故障排查](#故障排查)

## ✨ 特性

- ✅ **规范化测试结构** - 使用 testify/suite 组织测试
- ✅ **强类型设计** - 所有 API 模型都有明确的类型定义
- ✅ **环境变量配置** - 无硬编码，支持多环境
- ✅ **自动化清理** - 每个测试后自动清理资源
- ✅ **详细日志** - 可配置的详细输出
- ✅ **错误处理** - 统一的错误处理和重试机制

## 📦 环境要求

- **Go**: 1.21 或更高版本
- **网络**: 能够访问测试服务器
- **设备**: 至少一台在线的 BKV 测试设备

## 🚀 快速开始

### 1. 进入目录

```bash
cd test/e2e
```

### 2. 安装依赖

```bash
go mod download
```

### 3. 配置环境变量

```bash
# 方式1: 复制并编辑 .env 文件
cp .env.example .env
# 编辑 .env 文件

# 方式2: 直接设置环境变量
export E2E_SERVER_URL="http://182.43.177.92:7055"
export E2E_API_KEY="your_api_key_here"
export E2E_DEVICE_ID="82241218000382"
```

### 4. 运行测试

```bash
# 运行所有测试
go test -v ./...

# 运行特定套件
go test -v -run ChargeSuite

# 详细输出
E2E_VERBOSE=true go test -v ./...
```

## ⚙️ 配置

### 必填环境变量

| 变量名          | 说明        | 示例                         |
| --------------- | ----------- | ---------------------------- |
| `E2E_API_KEY`   | API 密钥    | `sk_test_thirdparty_key_...` |
| `E2E_DEVICE_ID` | 测试设备 ID | `82241218000382`             |

### 可选环境变量

| 变量名                | 默认值                  | 说明       |
| --------------------- | ----------------------- | ---------- |
| `E2E_SERVER_URL`      | `http://localhost:7055` | 服务器地址 |
| `E2E_REQUEST_TIMEOUT` | `30s`                   | 请求超时   |
| `E2E_WAIT_TIMEOUT`    | `60s`                   | 等待超时   |
| `E2E_VERBOSE`         | `false`                 | 详细输出   |

## 🧪 测试套件

### ChargeSuite - 充电流程测试

```bash
go test -v -run ChargeSuite
```

测试用例：

- ✅ 端口 1/2 充电完整流程
- ✅ 双端口并发充电
- ✅ 端口冲突处理
- ✅ 不同充电模式
- ✅ 停止充电指令
- ✅ 无效参数处理

### DeviceSuite - 设备管理测试

```bash
go test -v -run DeviceSuite
```

测试用例：

- ✅ 获取设备信息
- ✅ 设备在线状态
- ✅ 设备不存在处理
- ✅ 端口信息查询
- ✅ 活跃订单追踪

### OrderSuite - 订单管理测试

```bash
go test -v -run OrderSuite
```

测试用例：

- ✅ 订单信息查询
- ✅ 订单状态流转
- ✅ 订单不存在处理
- ✅ 时间追踪
- ✅ 电量追踪

## 📊 CI/CD 集成

### GitHub Actions 示例

```yaml
name: E2E Tests
on: [push, pull_request]

jobs:
  e2e:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3
      - uses: actions/setup-go@v4
        with:
          go-version: "1.21"
      - name: Run Tests
        working-directory: test/e2e
        env:
          E2E_SERVER_URL: ${{ secrets.E2E_SERVER_URL }}
          E2E_API_KEY: ${{ secrets.E2E_API_KEY }}
          E2E_DEVICE_ID: ${{ secrets.E2E_DEVICE_ID }}
        run: go test -v -timeout 10m ./...
```

## 🐛 故障排查

### 常见问题

**1. 设备不在线**

```
错误: timeout waiting for device online
解决: 确认设备ID正确，检查设备在线状态
```

**2. API 密钥错误**

```
错误: API Error [401]: Unauthorized
解决: 检查 E2E_API_KEY 环境变量
```

**3. 充电未启动**

```
现象: 订单一直为 pending 状态
解决: 确保充电器已插入对应端口
```

### 调试技巧

```bash
# 启用详细日志
E2E_VERBOSE=true go test -v -run ChargeSuite/TestPort1Charge

# 查看完整错误
go test -v ./... 2>&1 | tee test.log

# 增加超时时间
E2E_WAIT_TIMEOUT=120s go test -v ./...
```

## 📁 项目结构

```
test/e2e/
├── config.go           # 配置管理
├── client.go           # HTTP 客户端
├── models.go           # 数据模型
├── helpers.go          # 测试助手
├── charge_test.go      # 充电测试
├── device_test.go      # 设备测试
├── order_test.go       # 订单测试
├── README.md           # 本文档
├── .env.example        # 配置示例
└── go.mod              # 依赖管理
```

## 📝 更新日志

### v2.0.0 (2025-10-31)

- ✨ 完全重构为规范测试套件
- ✨ 使用 testify/suite
- ✨ 强类型 API 模型
- ✨ 环境变量配置
- ✨ 自动资源清理
- 📝 完善文档

---

**Happy Testing! 🎉**
